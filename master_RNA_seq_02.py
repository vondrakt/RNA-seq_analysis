...
if options.path is None or options.samples is None or options.replicates is None\
        ...
        options.annotation_gff is None or options.config_DE is None or options.count_option is None:
...
if options.count_option == 'GENE':
    ...
    DE_analysis_command_edgeR = 'run_DE_analysis.pl --matrix transcript_count_matrix.tsv --samples_file %s ' \
                                '--reference_sample condition1 --method edgeR --output edgeR_genes' % options.config_DE
    print(DE_analysis_command_edgeR)
    os.system(DE_analysis_command_edgeR)

    DE_analysis_command_DESeq2 = 'run_DE_analysis.pl --matrix transcript_count_matrix.tsv --samples_file %s ' \
                                 '--reference_sample condition1 --method DESeq2 --output DESeq2_genes' % options.config_DE
    print(DE_analysis_command_DESeq2)
    os.system(DE_analysis_command_DESeq2)
    ...
    print(new_dir)
    os.chdir(new_dir)
    analyze_DE = 'analyze_diff_expr.pl --matrix ../transcript_count_matrix.tsv --samples %s -P 0.05 -C 2' % options.config_DE
    print(analyze_DE)
    os.system(analyze_DE)
